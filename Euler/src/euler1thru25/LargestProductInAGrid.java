...
public class LargestProductInAGrid {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 20; j++) {
				numbers[i][j] = Integer.parseInt(in.next());
			}
		}
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 20; j++) {
				...
				long product4 = 1;
				for (int k = 0; k < 4; k++) {
					if (i < 17 && j < 17) product2 *= numbers[i + k][j + k];
					if (i < 17 && j < 15) product4 *= numbers[i + k][19 - j - k];
					if (j < 17) product *= numbers[i][j + k];
					if (i < 17 ) product1 *= numbers[i + k][j];
				}
